...
class Button():
    ...
    def update_scale(self):
        if self.factor_w != gameDisplay.factor_w:
            ...
            self.x_scaled = self.x * self.factor_w
            self.w_scaled = self.w * self.factor_w
            
        if self.factor_h != gameDisplay.factor_h:
            self.factor_h = gameDisplay.factor_h
            self.y_scaled = self.y * self.factor_h
            self.h_scaled = self.h * self.factor_h
...
class Button_Image():
    ...
    def update_scale(self):
        ...
        if self.center == False:
            self.rect_scaled = self.active.get_rect(topleft=(self.x_scaled, self.y_scaled))

        if self.center == True:
            self.rect_scaled = self.active.get_rect(center=(self.x_scaled, self.y_scaled))
...
def Title_Screen():
    ...
    Button_Image(300, 25, True, button_exit_1, button_exit_2, None, Title_Screen)
...
class PlayerIG():
    ...
    def display_card(self):
        ...
        for index in range(5):
            gameDisplay.blit(self.base_card[PlayerIG.card[0][index][0]], (120+65*index, 480))
