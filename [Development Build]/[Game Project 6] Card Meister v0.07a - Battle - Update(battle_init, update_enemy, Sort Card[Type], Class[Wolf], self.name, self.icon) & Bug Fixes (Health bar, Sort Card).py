...
class MainIG():
    ...
    def update_enemy(self, enemy):
        ...
        self.resilience[1]  = enemy.resilience
    ...
    def update_card(self):
        ...
        for side in range(2):
            ...
            self.card[side] = sorted(self.card[side], key=itemgetter(1), reverse=True)
                ...
                # Sort Type Card
            self.card[side] = sorted(self.card[side], key=itemgetter(0), reverse=True)
    ...
    def element_update(self):
        ...
        for side in range(2):
            ...
            if self.board[side] != []:
                ...
                element_type[side] = card_list.index(max(card_list))
                self.board_element[side]    = self.base_board[element_type[side]]
                self.banner_element[side]   = self.base_banner[element_type[side]]
        ...
        """
        if self.board[0] != []:
            p_type, e_type = element_type[0], element_type[1]
    ...
    def update(self):
        ...
        # Health Bar
        pygame.draw.rect(gameDisplay, Color_Green, (510, 530, 100*1.5*self.health[0]/self.maxhealth[0], 30))
        pygame.draw.rect(gameDisplay, Color_Green, (140, 80,  100*1.5*self.health[1]/self.maxhealth[1], 30))
