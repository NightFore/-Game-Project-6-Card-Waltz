    ...
    def update(self):
        ...
        for event in Tools.events:
            if Tools.event.type == pygame.MOUSEBUTTONDOWN and Tools.event.button == 3:
                len_played = [i for i in range(len(PlayerIG.played)) if PlayerIG.played[i] != None]
                if len_played != []:
                    PlayerIG.unselect_card(max(len_played))
PlayerIG = PlayerIG()
