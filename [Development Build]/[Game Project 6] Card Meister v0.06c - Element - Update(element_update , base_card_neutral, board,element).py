...
def Main_Screen():
    ...
    # Text
    Button(None, None, 125, 200, 100, 200, 10, True, False, Color_Green, Color_Red, None, MainIG.update_card)
...
class MainIG():
    ...
    def battle_init(self):
        for side in range(2):
            self.health[side] = self.maxhealth[side]
        Text("Name Placeholder", 140, 50, False, Text_interface)
        Text("Name Placeholder", 450, 500, False, Text_interface)
        
        Button_Image(55,  480, False, base_card_ok_inactive, base_card_ok_active, None, self.element_update)
        self.update_card()


    def element_update(self, side=0):
        card_list = [[], [], []]

        if self.board[side] != []:
            for index in range(len(self.board[side])):
                card_type, card_level = self.card[side][self.board[side][index]][0], self.card[side][self.board[side][index]][1]
                card_list[card_type].append(card_level)
                
            for index in range(3):
                card_list[index] = sum(card_list[index])

            element_advantage = card_list.index(max(card_list))

            self.board_element[side] = self.base_board[element_advantage]

            print(card_list)
            print(element_advantage)
    ...
    def update_card(self):
        ...
        self.element_update(0)
        self.element_update(1)
    ...
    def update(self):
        ...
        # Board Type Advantage
        gameDisplay.blit(self.board_element[0], (240,305))
        gameDisplay.blit(self.board_element[1], (240,205))

        # Health Bar
        pygame.draw.rect(gameDisplay, Color_Green, (510, 530, 1.5*self.health[0], 30))
        pygame.draw.rect(gameDisplay, Color_Green, (140, 80,  1.5*self.health[1], 30))
        ...
        for side in range(2):
            ...
            # Hand - Enemy/Empty Card
                if self.hand[side][index] == None:
                    gameDisplay.blit(base_card_empty, (120+65*index+300*side, 480-450*side))
