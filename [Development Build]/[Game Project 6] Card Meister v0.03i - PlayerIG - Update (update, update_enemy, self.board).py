...
base_card_board         = pygame.image.load("Data\Graphics\Base_card_board.png")
...
button_exit_1           = pygame.image.load("Data\Graphics\Button_exit_1.png").convert()
button_exit_2           = pygame.image.load("Data\Graphics\Button_exit_2.png").convert()
...
def Title_Screen():
    ...
    # Text
    Button(None, None, 125, 200, 100, 200, 10, True, False, Color_Red, Color_Green, None, action=PlayerIG.update_card)
...
class PlayerIG():
    def __init__(self):
        ...
        self.played     = [[None, None, None, None, None], [None, None, None, None, None]]
        self.init       = False
    ...
    def select_card(self, index):
        if self.hand[0][index] in Setup.list_button_image:
            ...
            self.played[0][index] = Button_Image(240+65*index, 305, False, card, card, index, self.unselect_card)


    def unselect_card(self, index):
        if self.played[0][index] in Setup.list_button_image:
            Setup.list_button_image.remove(self.played[0][index])
            self.played[0][index] = None
    ...
    def update_card(self):
        ...
        for index in range(5):
            ...
            for side in range(2):
                ...
                level = self.base_level[type_card] + random.randint(0, 3)
                self.card[side][index]  = [type_card, level]

        # Sort by card type
        self.card[0] = sorted(self.card[0], key=itemgetter(0))
        self.card[1] = sorted(self.card[1], key=itemgetter(0))
    ...
    def update_enemy(self):
        self.played[1] = [None, None, None, None, None]
        
        for index in range(5):
            if len(self.board[1]) < 4:
                play_card = random.choice([True, False])

                if play_card == True:
                    self.played[1][index]  = self.hand[1][index]
                    self.hand[1][index]    = None
                    
            self.board[1] = [i for i in range(len(self.played[1])) if self.played[1][i] != None]

        
    def update(self):
        gameDisplay.blit(icon_status_iris,      (670, 485))
        gameDisplay.blit(icon_status_direwolf,  (50,  35))
        gameDisplay.blit(base_card_board,       (235, 200))
        ...
        for index in range(len(self.board[1])):
            gameDisplay.blit(self.base_card[self.card[1][self.board[1][index]][0]], (305+65*index, 205))
        ...
        # Remove the rightmost card
        for event in Tools.events:
            print(self.board[0])
            if Tools.event.type == pygame.MOUSEBUTTONDOWN and Tools.event.button == 3:
                len_played = [i for i in range(len(self.played[0])) if self.played[0][i] != None]
                if len_played != []:
                    self.unselect_card(max(len_played))
PlayerIG = PlayerIG()
